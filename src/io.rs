...
pub fn read_console_output_attribute<'a>(console_output: &mut impl AsConsoleOutputHandle, attributes: &'a mut [Attributes], read_coord: impl IntoCoord) -> io::Result<&'a [Attributes]> {
    let console_output = console_output.as_raw_handle().cast();
...
pub fn read_console_output_character<'a>(console_output: &mut impl AsConsoleOutputHandle, characters: &'a mut [u16], read_coord: impl IntoCoord) -> io::Result<&'a [u16]> {
    let console_output = console_output.as_raw_handle().cast();
